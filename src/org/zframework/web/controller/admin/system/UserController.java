...
public class UserController extends BaseController<User>{
	...
	public Map<String,Object> userList(PageBean pageBean,String[] name,String[] value){
		...
		//过滤已添加微信关注,但未审核通过的管理员用户
		pageBean.addCriterion(Restrictions.lt("enabled",2));
	...
	public String toAdd(Model model){
		...
		List<User> users = userService.list(Restrictions.and(Restrictions.eq("enabled",2)));
		JSONArray userArray = new JSONArray();
		for(User user : users){
			JSONObject juser = new JSONObject();
			juser.element("nickName", user.getNickName());
			juser.element("openId",user.getOpenId());
			userArray.add(juser);
		}
		model.addAttribute("users",userArray.toString());
	...
	public JSONObject doAdd(HttpSession session,HttpServletRequest request,@Valid @ModelAttribute("user")User user,BindingResult result){
		if(result.hasErrors()){
			...
			User eqUser = userService.getByOpenId(user.getOpenId());//根据微信OPENID获取用户，
			//记录日志
			User currentUser = (User) session.getAttribute(ApplicationCommon.SESSION_ADMIN);
			if(ObjectUtil.isNotNull(eqUser)){
				user.setId(eqUser.getId());
				//获取选择的角色
				...
				if(StringUtil.isEmpty(userUnit)){
					...
					//	unitList.add(unit);
					//}else{
						return WebResult.error("请添加站点信息!");
					//}
				}else{
				...
				user.setEnabled(0);
				userService.update(user);//保存到数据库中
				logService.recordInfo("新增用户","成功",currentUser.getLoginName() , request.getRemoteAddr());
				return WebResult.success();
			}else{
				logService.recordInfo("新增用户","失败（微信未关注服务公众号）",currentUser.getLoginName() , request.getRemoteAddr());
				return WebResult.error("微信未关注服务公众号!");
			}
		}
	}
...
//				updateUser.setWeb(user.getWeb());
	...
	@RequestMapping(value="/assignUnit/{id}",method={RequestMethod.GET})
	public String toAssignUnit(Model model,@PathVariable Integer id ){

		User user = userService.getById_NoLazy(id);
		List<Unit> list = user.getUnits();
		Iterator<Unit> it = list.iterator();
		List<Unit> baseList = new ArrayList<Unit>();
		//排除掉不是站级的unit
		while(it.hasNext()){
			Unit x = it.next();
			if(!ApplicationCommon.TYPE_STATION.equals(x.getUnitType())){
				baseList.add(x);
				it.remove();
			}
		}
		Integer baseId;
		if(baseList.size() == 0){
			baseId = list.get(0).getId();
		}else {
			baseId = baseList.get(0).getId();
		}
		...
		model.addAttribute("unitIds", StringUtil.toString(ids));
		return "admin/system/user/assignUnit";
	}
	@RequestMapping(value="/doAssignUnit",method={RequestMethod.POST})
	@ResponseBody
	public JSONObject doAssignUnit(HttpServletRequest request,Integer userId,Integer[] selectedIds){
		if(ObjectUtil.isNotEmpty(selectedIds)){
			List<Unit> selectedUnits = unitService.getByIds(selectedIds);
			User user = userService.getById_NoLazy(userId);
			List<Unit> oldUnitList= user.getUnits();
			//参数传递进来的都是站点单位,要将原有的非站点单位的省级,市级单位加进来update
			for(Unit unit:oldUnitList){
				if (!ApplicationCommon.TYPE_STATION.equals(unit.getUnitType())){
					selectedUnits.add(unit);
				}
			}
			user.setUnits(selectedUnits);
			userService.update(user);
			logService.recordInfo("分配站点","成功", getCurrentUser().getLoginName(), request.getRemoteAddr());
			return WebResult.success();
		}else{
			User user = userService.getById(userId);
			user.setUnits(null);
			userService.update(user);
			logService.recordInfo("分配站点","成功(删除所有分配站点)", getCurrentUser().getLoginName(), request.getRemoteAddr());
			return WebResult.success();
		}
	}
}

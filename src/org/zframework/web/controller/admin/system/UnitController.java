...
public class UnitController extends BaseController<Unit> {
	...
	@ResponseBody
	public JSONArray getUnitTree(@RequestParam(value="id",required=false) Integer parentId){
		
		return unitService.getUnitTree(parentId, this.getCurrentUser());
	}
...
//	}
	...
	/***
	 * 获取用户站点列表
	 * @param baseId  用户最上层机构ID
	 * @param isEq  是否相等判断条件
	 * @param unitIds
	 * @return
	 * */
	@RequestMapping(value="/assignUnitListForUser",method={RequestMethod.POST,RequestMethod.GET})
	@ResponseBody
	public Map<String,Object> assignUnitListForUser(Integer baseId, boolean isEq, Integer[] unitIds){

		Map<String,Object> unitMap = new HashMap<String, Object>();
		List<Unit> unitList = new ArrayList<Unit>();
		if (isEq){
			if(ObjectUtil.isNotNull(unitIds)&& ObjectUtil.isNotEmpty(unitIds)){
				unitList = unitService.getByIds(unitIds);
			}
		}else {
			unitList = unitService.getAllChildUnitList_NoLazyUser(baseId);
			Iterator<Unit> it = unitList.iterator();
			while(it.hasNext()){
				Unit x = it.next();
				if(!ApplicationCommon.TYPE_STATION.equals(x.getUnitType()) || x.getUsers().size() > 0){
					it.remove();
					continue;
				}
		...
		unitMap.put("rows",unitList);
		unitMap.put("total",unitList.size());
		return  unitMap;
	}

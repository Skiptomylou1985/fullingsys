...
//初始化按钮权限
if(typeof _InitButtonPrrmission == "function"){
...
_248.window.draggable({handle:">div.panel-header>div.panel-title",disabled:_248.options.draggable==false,onStartDrag:function(e){
	if(_248.options.proxy == true){
		...
		_248.options.isDrag = true;
	}
},onDrag:function(e){
	if(_248.options.proxy){
		_248.proxy.css({display:"block",left:e.data.left,top:e.data.top});
	}else{
		if(_248.options.isDrag){
...
//新增加一个 proxy（拖动时是否显示代理）
//isDrag 当前是否处于拖拽状态
$.fn.window.defaults=$.extend({},$.fn.panel.defaults,{zIndex:9000,draggable:true,resizable:true,shadow:true,modal:false,inline:false,title:"New Window",collapsible:true,minimizable:true,maximizable:true,closable:true,closed:false,proxy:true,isDrag:false});
})(jQuery);
(function($){
function _252(_253){
...
_279[$.messager.defaults.ok]=function(){
	//新增判断okEvent
	if($.messager.defaults.okEvent != null){
		$.messager.defaults.okEvent();
	}
win.window("close");
if(fn){
fn();
...
_27c[$.messager.defaults.cancel]=function(){
	//新增判断cancelEvent
	if($.messager.defaults.cancelEvent != null){
		$.messager.defaults.cancelEvent();
	}
win.window("close");
if(fn){
fn(false);
...
return win;
},prompt:function(_27d,msg,fn,type){
	//添加onKeyDown事件
var _27e="<div class=\"messager-icon messager-question\"></div>"+"<div>"+msg+"</div>"+"<br/>"+"<div style=\"clear:both;\"/>"+"<div><input class=\"messager-input\" type=\"text\" onKeyDown=\"if(event.keyCode==13){($(this).parent().parent().find('a').get(0)).click()}\"/></div>";
if(type == "password")//添加密码验证
	_27e="<div class=\"messager-icon messager-question\"></div>"+"<div>"+msg+"</div>"+"<br/>"+"<div style=\"clear:both;\"/>"+"<div><input class=\"messager-input\" type=\"password\" onKeyDown=\"if(event.keyCode==13){($(this).parent().parent().find('a').get(0)).click()}\"/></div>";
var _27f={};
...
//新增okEvent cancelEvent
$.messager.defaults={ok:"Ok",cancel:"Cancel",okEvent:null,cancelEvent:null};
})(jQuery);
(function($){
function _284(_285){
...
//添加form请求类型类型判断
var _f = $("<input type=\"hidden\" name=\"_SYSTEMREQUEST_FROM_000\" value=\"true\">").appendTo(form);
_437 = _437.add(_f);
_438();
form[0].submit();
}
...
//添加onclick方法，点击输入框自动弹出日期选择框
var span = $("<span class=\"combo\">"+"<input type=\"text\" class=\"combo-text\" autocomplete=\"off\">"+"<span><span class=\"combo-arrow\"></span></span>"+"<input type=\"hidden\" class=\"combo-value\">"+"</span>");
if(_7fb["className"].indexOf("easyui-datebox")>=0)
    span=$("<span class=\"combo\">"+"<input type=\"text\" class=\"combo-text\" autocomplete=\"off\" onclick=\"$(this).parent().find('span>span').click();\" onblur=\"if(!Validate.date(this.value) && jQuery.trim(this.value).length>0){top.Notiy.warning('日期格式输入错误');this.value='';}\" onkeyup=\"JS.importPluginJS('util/DateUtil.js');DateUtil.format(this);\">"+"<span><span class=\"combo-arrow\"></span></span>"+"<input type=\"hidden\" class=\"combo-value\">"+"</span>");
